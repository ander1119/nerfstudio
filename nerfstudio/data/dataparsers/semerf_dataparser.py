...
@dataclass
class SemerfDataParserConfig(DataParserConfig):
    """Instant-NGP dataset parser config"""

    _target: Type = field(default_factory=lambda: Semerf)
...
@dataclass
class Semerf(DataParser):

    config: SemerfDataParserConfig
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        for frame in meta["frames"]:
            ...
            if not fname.exists():
                ...
                if "depth_path" in frame:
                    depth_fname = data_dir / Path(frame["depth_path"])
                    depth_filenames.append(depth_fname)
                if "semantic_path" in frame:
                    semantic_fname = data_dir / Path(frame["semantic_path"])
        ...
        fl_x, fl_y = Semerf.get_focal_lengths(meta)
        ...
        if len(semantic_filenames) != 0:
            semantic_info = meta['semantic_info']
            classes = semantic_info['class_names']
            colors = torch.tensor(semantic_info['class_colors'], dtype=torch.float32) / 255.0
